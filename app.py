...
def generar_codigo_colab(config, umbrales, overrides, start=None, end=None):
    rango = ""
    if start and end:
        rango = ("\n# 4) Rango de fechas seleccionado (pégalo en la Parte 1.3 del notebook):\n"
                 f'START = "{start}"\nEND   = "{end}"\n')
...
def ccf_matriz(df, variables, kpi_col, max_lag=8):
    """Correlación de Spearman de cada variable (desplazada) con el KPI, por rezago."""
    y = df[kpi_col]
...
def fig_ccf_heatmap(m):
    ...
    ax.set_title("Correlación cruzada con el KPI por rezago", color=NAVY, fontweight="bold")
    ax.set_xlabel("Rezago (semanas)"); ax.set_ylabel("")
    fig.tight_layout()
    return fig

def fig_control_trends(df, ctrl, date_col):
    d = df.copy(); d[date_col] = pd.to_datetime(d[date_col]); d = d.sort_values(date_col)
    n = len(ctrl); ncols = min(2, n) or 1; nrows = int(np.ceil(n / ncols))
    fig, axes = plt.subplots(nrows, ncols, figsize=(6 * ncols, 3 * nrows), squeeze=False)
    axes = axes.reshape(-1)
    for ax, c in zip(axes, ctrl):
        ax.plot(d[date_col], d[c], color=NAVY, lw=1.8)
        ax.fill_between(d[date_col], d[c], alpha=0.07, color=NAVY)
        ax.set_title(c, fontsize=10)
    for ax in axes[n:]:
        ax.axis("off")
    fig.suptitle("Tendencia de las variables de control", color=NAVY, fontweight="bold", y=1.02)
    fig.tight_layout()
    return fig

def control_vs_kpi(df, ctrl, kpi_col):
    y = df[kpi_col]
    mi = mutual_info_regression(df[ctrl].fillna(df[ctrl].median()).values, y.values, random_state=0)
    rows = []
    for i, c in enumerate(ctrl):
        blag, bcorr = _ccf_best(df[c], y)
        rows.append({"control": c, "corr_KPI": round(df[c].corr(y, method="spearman"), 3),
                     "MI_KPI": round(float(mi[i]), 3), "mejor_lag": blag, "corr_en_lag": bcorr})
...
<div>Generado el {meta['fecha']} · Periodo: {meta.get('periodo','—')} · {meta['filas']} semanas · {meta['medios']} medios · {meta['controles']} controles</div></div>
...
if fmin >= fmax:
    ...
    rango = st.slider("Arrastra los extremos para acotar el periodo que quieres revisar",
                      min_value=fmin, max_value=fmax, value=(fmin, fmax), format="YYYY-MM-DD")
...
cf1.metric("Semanas seleccionadas", len(df_model))
cf2.metric("Desde", r0.strftime("%Y-%m-%d"))
cf3.metric("Hasta", r1.strftime("%Y-%m-%d"))
if len(df_model) < 5:
    st.error("El rango tiene muy pocos datos (< 5 semanas). Amplía el periodo para poder analizar.")
    st.stop()
if len(df_model) < 20:
...
with t_model:
    ...
    st.caption("Correlación de cada variable con el KPI a distintos rezagos (semanas). "
               "La casilla más intensa por fila indica el rezago donde más se relaciona: pista del adstock.")
    incluir_ctrl = st.checkbox("Incluir variables de control en la matriz", value=True)
    vars_ccf = media_cols + (ctrl_cols if incluir_ctrl and ctrl_cols else [])
    m_ccf = ccf_matriz(df_model, vars_ccf, kpi_col, max_lag=8)
    figM = fig_ccf_heatmap(m_ccf)
    st.pyplot(figM); plt.close(figM)

    st.subheader("🎛️ Variables de control")
    if not ctrl_cols:
        st.info("No seleccionaste variables de control. Añádelas en la Configuración de campos "
                "para ver su tendencia y su relación con el KPI.")
    else:
        st.markdown("**Relación con el KPI**")
        st.caption("Un control muy correlacionado con el KPI (p. ej. población o estacionalidad) "
                   "es clave para que el modelo no le atribuya a los medios lo que en realidad "
                   "explica el contexto.")
        cvk = control_vs_kpi(df_model, ctrl_cols, kpi_col)
        st.dataframe(cvk.round(3), use_container_width=True)
        for _, r in cvk.iterrows():
            if abs(r["corr_KPI"]) >= 0.4:
                st.success(f"**{r['control']}**: fuerte relación con el KPI ({r['corr_KPI']}). "
                           "Buen control, mantenlo en el modelo.")
            elif abs(r["corr_KPI"]) >= 0.15 or abs(r["corr_en_lag"]) >= 0.2:
                st.info(f"**{r['control']}**: relación moderada ({r['corr_KPI']}; "
                        f"mejor rezago {int(r['mejor_lag'])} → {r['corr_en_lag']}).")
            else:
                st.warning(f"**{r['control']}**: casi no se relaciona con el KPI ({r['corr_KPI']}). "
                           "Revisa si aporta como control o si conviene quitarlo.")
        st.markdown("**Tendencia en el tiempo**")
        figC = fig_control_trends(df_model, ctrl_cols, date_col)
        st.pyplot(figC); plt.close(figC)
...
with t_code:
    ...
    codigo = generar_codigo_colab(CONFIG, UMBRALES, overrides, FECHA_INI, FECHA_FIN)
